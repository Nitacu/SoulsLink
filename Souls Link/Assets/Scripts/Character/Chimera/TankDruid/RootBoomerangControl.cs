...
public class RootBoomerangControl : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Enemy"))
        {
            ...
            collision.gameObject.GetComponent<SimpleEnemyController>().GetRooted(rootDuration);
            GameObject temp = Instantiate(_rootPrefab, gameObject.transform.position, Quaternion.identity);
            StartCoroutine(destroyRoot(rootDuration, temp));
        }
    ...
    IEnumerator destroyRoot(float time, GameObject root)
    {
